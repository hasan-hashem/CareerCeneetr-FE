        ...
        this.subs.add(this.profileService.updateMyService(this.id,this.edit).subscribe(res => 
            {
                ...
                this.toastr.success('Service data has been updated successfully', 'DelveHub ^_^');
            }
        ));
    }
    stopService() {
        this.profileService.patchMyService(this.id,this.patchValue).subscribe(res => {
            this.closeModal.emit(true)
            this.toastr.success('The service has been stopped successfully', 'DelveHub ^_^');
        });
    }
    resumeService() {
        this.profileService.patchMyService(this.id,this.patchValue).subscribe(res => {
            this.closeModal.emit(true)
            this.toastr.success('The service has been successfully resumed', 'DelveHub ^_^');
        });
    }
    deleteService() {
        this.profileService.deleteMyService(this.id).subscribe(res => {
            this.closeModal.emit(true);
            this.toastr.success('The service has been deleted successfully', 'DelveHub ^_^');
        });
    }
