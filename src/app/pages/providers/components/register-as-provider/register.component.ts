...
export class RegisterComponent implements OnInit, OnDestroy {
    ...
    submitProvider() {
        ...
        this.subs.add(
            this.subs.add(this.providerService.addProvider(this.serviceId,this.providerModel).subscribe(
                {
                    next: (response) => {
                        this.route.navigate(['/me/portfolio']);
                        this.toastr.success('A new service has been successfully registered. The -+- DelveHub -+- team wishes you success :)', 'DelveHub ^_^');
                    },
                    error: (error) => {
    ...
    getServices() {
      this.subs.add(this.providerService.getServicesList().subscribe(res => {
        ...
        this.filteredServices = this.servicesList.filter(x => x.categoryId == this.cateroryList.id)
      }))
    }
