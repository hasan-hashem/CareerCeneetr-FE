...
export class DialogProjectComponent {
  ...
  submitProject() {
      if (this.projectForm.valid) {
          ...
          const date = new Date();
          this.project.createdDate = date.toLocaleDateString();
          this.projectService.addProject(this.project).subscribe({
              next: () => {
                  this.toastr.success('Your project has been successfully added.', 'Career Center');
              },
              error: () => {
                  this.toastr.error('An error occurred while adding your project.', 'Career Center');
              }
          });
      } else {
